...
export default function LeaderboardPage() {
  const { player, loading } = useTelegram();
  ...
  }, [loading, telegramId, mode, loadBoard]);

  return (
    <main className="mx-auto min-h-screen max-w-xl px-4 py-6">
      <header className="mb-6 flex items-center justify-between">
        <BrandMark href="/app" />
        <Link href="/app" className="font-pixel text-[9px] text-cyan">
          ← BACK
        </Link>
      </header>
      ...
      <CrtPanel label="LEADERBOARD // ARCADE RANKINGS" tone="amber">
        ...
        <div className="mt-4 flex items-center gap-2 text-ash">
          ...
          <span className="font-term text-[14px]">
            Points convert to $GLEAN at launch.
          </span>
        </div>
      </CrtPanel>
    </main>
