import { supabase } from '../supabase.js';
import { unwrap } from '../lib/errors.js';
import { buildMiniAppPath } from '../lib/links.js';

export async function ghostCommand(ctx) {
  const telegramId = ctx.from?.id;
  if (!telegramId) return;

  const user = unwrap(
    await supabase
      .from('users')
      .select('id, wallet_address, points')
      .eq('telegram_id', telegramId)
      .maybeSingle(),
    'ghostCommand:user'
  );

  if (!user) {
    await ctx.reply('Run /start first to register.');
    return;
  }

  const walletLine = user.wallet_address
    ? `Wallet: ${user.wallet_address.slice(0, 4)}…${user.wallet_address.slice(-4)}`
    : 'No wallet linked yet — connect Phantom in the app first.';

  const url = buildMiniAppPath('ghost-race');

  await ctx.reply(
    `👻 Ghost Race\n\n${walletLine}\n\nSend a real Solana memo tx and race ETH/BTC ghosts on confirmation speed.`
  );
  await ctx.reply('👻 Ghost Race', {
    reply_markup: {
      inline_keyboard: [[{ text: '👻 Start Ghost Race', web_app: { url } }]],
    },
  });
}
